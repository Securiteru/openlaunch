# ToolPilot

| | |
|---|---|
| Site | https://www.toolpilot.ai/ |
| Category | ai-directory |
| Product page pattern | UNVERIFIED |
| Cost | UNVERIFIED — check for paid tiers before submitting; pause before any payment |
| Account required | UNVERIFIED |
| Listing status flow | UNVERIFIED |

## Preparation
- [ ] Product brief fields ready: name, tagline, description, category, links, logo, screenshots
- [ ] Account exists — create one only with explicit user approval
- [ ] Free vs. paid submission path identified on-site

## Submission steps
Steps below are a generic skeleton. Verify each one on the live site and
correct this file when reality differs — that is how this playbook improves.

1. Find the submission entry point ("Submit", "Add product", "List your tool",
   or a pricing/submit page).
2. Fill every listing field from the product brief — approved claims only.
3. Check the hurdles section, then submit.
4. Record the outcome in `tracker/data.json` with evidence.

## Known hurdles
UNVERIFIED — nothing recorded yet. As observed, record: CAPTCHAs, email
verification, moderation queues, paid-only placement, badge requirements,
dofollow policy, review turnaround time.

## Confirmation
UNVERIFIED — expected confirmation signal (email, dashboard entry, public
URL) not yet recorded. A listing counts as `live` only when its public URL
loads with the product visible.

## Badge
Platform provides an embeddable badge. Record the snippet in this file after the listing is live.


## Field observations
Source: LaunchRepo public field guide (https://launchrepo.dev/directories/toolpilot/), observed unknown.

ToolPilot is an AI tools directory and review platform organised in more than twenty categories — text, image, video, coding, chatbots and more. Each product page shows vendor, description, price, star rating and badges such as Editor’s Pick or Free Trial.

Checked on September 17, 2026 on the platform’s public pages.

### Who lists here

Makers of AI products who want a category listing with a review section, and who can place a ToolPilot badge or logo on their site to qualify for the free tier.

### What to prepare

The submit page states that a link back to ToolPilot is mandatory for all free listings and must be confirmed via URL or email after adding one of their badges.
Product details as shown on existing pages: name, category, description, pricing model.
A look at the pricing table, which is published as an image on the submit page; the numbers are not readable as text.

### What “live” means here

Live means a page under /products/ that shows up in its category with the rating widget. The public pages show no review timeline, so check for the page directly.

### Worth knowing

A login link exists in the header, but the submit page itself does not say an account is required; be ready for either.
Official site: ToolPilot ↗All ai tool directories →
Short profile from the platform’s public pages, not a submission service. Rules, prices and requirements change — recheck before you submit. LaunchRepo makes no publication, backlink or ranking promise.

###

Submission entry: https://www.toolpilot.ai/

### Submit — observed 2026-09-19 (BrowserOS, field-level)

- `/pages/submit-your-ai-tool` is a **Shopify contact-form flow** (hidden `contact[tags]` + email). AI-only, weak fit.
